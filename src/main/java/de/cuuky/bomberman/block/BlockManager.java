...
public class BlockManager {
	...
	public BlockManager() {
		...
		for (String key : cfg.getKeys(true))
			savedBlocks.add((Location) cfg.get(key));
	}

	public void restore() {
		for (Location loc : savedBlocks)
			loc.getWorld().getBlockAt(loc).setType(Material.CLAY);
	}

	public void load(Location loc1, Location loc2) {
		savedBlocks.clear();
		changed = true;
		for (Block block : getBlocksBetweenPoints(loc1, loc2))
			savedBlocks.add(block.getLocation());
	}

	public void save() {
		if (savedBlocks.isEmpty() || !changed)
			return;
