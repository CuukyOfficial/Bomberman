...
public enum PowerUp {
	...
	public void addEnabled(Player player) {
		...
		switch (this) {
		case SHOCKWAVE:
			player.sendMessage(Bomberman.getPrefix()
					+ "§7Damit kannst du nun mit §eSneaken §7+ §eSchlagen §7deine Gegner um dich herum wegschleudern!");
			break;
		case INFINITE_TNT:
			player.sendMessage(
					Bomberman.getPrefix() + "§7Damit kannst du §eunendlich TnT §7hintereinander platzieren!");
			break;
		case SPEED:
	...
	public static void removeAllPowerUp(boolean silent) {
		...
		if (!silent)
			Bukkit.broadcastMessage(Bomberman.getPrefix() + Message.POWERUP_ALL_REMOVED.getMessage());
	}
}
