...
public class BombListener extends AbstractStateListenerJob {
    ...
    public void onPlayerAnimation(PlayerAnimationEvent event) {
        ...
        if (result != null && result.getHitEntity() != null) {
            ...
            bomb.getPrimed().setVelocity(newVelocity);

            player.getPlayer().playSound(player.getPlayer().getLocation(), Sound.ENTITY_IRON_GOLEM_ATTACK, 1.0f, 1.5f);
            player.getPlayer().getWorld().spawnParticle(Particle.CLOUD, bomb.getPrimed().getLocation(), 10, 0.2, 0.2, 0.2, 0.1);
        }
    }
}