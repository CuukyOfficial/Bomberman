...
public class ScoreboardSender {
    ...
    public static void sendScoreboard(Player p, int countdown) {
        ...
        if (Game.isUnlimitedTnTMode()) obj.getScore("§e-").setScore(4);
        else obj.getScore(getEvent(getNextEvent())).setScore(4);
    ...
    private static int getNextEvent() {
        int i = 0;
        if (Game.runningCountdown > 60) i++;
        if (Game.runningCountdown > 300) i++;
        if (Game.runningCountdown > 540) i++;
        return i;
    }
    ...
    public static void sendLobbyScore(Player p, String state) {
        if (p.getScoreboard().getEntries().contains(oldState.get(p))) p.getScoreboard().resetScores(oldState.get(p));
