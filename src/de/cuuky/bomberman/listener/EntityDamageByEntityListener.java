...
public class EntityDamageByEntityListener implements Listener {
	...
	public void onEntityDamage(EntityDamageByEntityEvent event) {
		...
		if (event.getDamager() instanceof TNTPrimed) {
			...
			if (player.getHealth() - event.getDamage() <= 0) {
				if (tnt.getShooter().getName().equals(player.getName())) {
					if (tnt.getLastTouched() == null)
						Bukkit.broadcastMessage(
								Message.PLAYER_DEATH_SUICIDE.getMessage().replaceAll("%player%", player.getName()));
					else if (!tnt.getLastTouched().getName().equals(player.getName())) {
						Bukkit.broadcastMessage(
								Message.PLAYER_DEATH_OWN_TNT.getMessage().replaceAll("%player%", player.getName())
										.replaceAll("%killer%", tnt.getLastTouched().getName()));
						playerKill(tnt.getLastTouched());
					}
				} else {
					Bukkit.broadcastMessage(
							Message.PLAYER_DEATH_KILLER.getMessage().replaceAll("%player%", player.getName())
									.replaceAll("%killer%", tnt.getLastTouched().getName()));
					playerKill(tnt.getLastTouched());
				}
				...
				player.setVelocity(tnt.getPrimedTnT().getLocation().toVector().subtract(player.getLocation().toVector())
						.normalize().multiply(2).setY(2));
			}
		} else if (event.getDamager() instanceof Player
				&& PowerUp.getPowerUp((Player) event.getDamager()) == PowerUp.PUNCH
				&& Bomberman.getState() == GameState.RUNNING) {
	...
	public static void playerKill(Player killer) {
		if (!kills.containsKey(killer.getUniqueId().toString()))
			kills.put(killer.getUniqueId().toString(), -1);
		...
		if (!Message.PLAYER_TAB.getMessage().isEmpty())
			killer.setPlayerListName(Message.PLAYER_TAB.getMessage().replaceAll("%player%", killer.getName())
					.replaceAll("%kills%", String.valueOf(kill)));
	}
}
