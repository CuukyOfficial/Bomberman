...
public class Utils {
    ...
    public static String getArgsToString(String[] args, String insertBewteen) {
        String command = "";
        for (String arg : args)
            if (command.equals("")) command = arg;
            else command = command + insertBewteen + arg;
        return command;
    }
    ...
    public static ItemStack getItemByString(String id) {
        ...
        if (!id.contains(":")) return new ItemStack(normalID);
        else return new ItemStack(Integer.valueOf(id.split(":")[0]), 1, Byte.valueOf(id.split(":")[1]));
    }
    ...
    public static int getNextToNine(int to) {
        ...
        while (true) {
            temp++;
            if (temp == 53) return temp;
            if (temp % 9 == 0) return temp;
        }
    }
